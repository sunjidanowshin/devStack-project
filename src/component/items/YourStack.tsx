...
const YourStack = ({addedItems, setAddedItems}: IYourStackProps) => {
  ...
  const handelRemove = (itemToRemove: Iitem)=>{
    ...
    return (
        <div className = "rounded-lg w-[280px] border border-gray-800 flex flex-col justify-between items-center">
      ...
      {addedItems.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {addedItems.map((item: Iitem, ind: number) => (
            <div key={ind} className="px-3 py-2 flex flex-rpw justytify- between  bg-gray-100 rounded-lg text-sm">
              {item.name}
